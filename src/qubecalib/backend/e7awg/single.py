...
class Driver:
    def __init__(
        ...
        self._awgctrl: Final[AwgCtrl] = AwgCtrl(self._ipaddr)
        self._capctrl: Final[CaptureCtrl] = CaptureCtrl(self._ipaddr)
    ...
    def capture_at(self) -> Future:
        """Caputre data when trigger is received"""
        with self._capctrl_lock:
            ...
            self._capctrl.start_capture_units(
                *[u for u in self._capus if u not in self._capus_with_triggers]
            )
        return self._executor.submit(
            self._capture_thread,
        )

    def capture_now(self) -> Results:
        """Capture data immediately without waiting for the trigger"""
        with self._capctrl_lock:
    ...
    def _load(self, settings: list[E7Setting]) -> None:
        with self._capctrl_lock, self._awgctrl_lock:
    ...
    def _get_dsp_units_enabled(self, capu: CaptureUnit) -> list[DspUnit]:
        ...
        return self._capctrl._CaptureCtrl__reg_access.read(
