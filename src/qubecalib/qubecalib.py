...
from .instrument.quel.quel1.driver.compat import (
    convert_settings_to_params,
    run,
)
...
class Converter:
    ...
    def _calc_modulation_frequency(
        ...
        if target_freq is None:
            raise ValueError("target frequency is not defined")
...
class Sequencer(Command):
    ...
    @property
    def driver(self) -> direct.Quel1System:
        # if self._driver is None:
        #     raise ValueError("driver is not defined")
        return self._driver
    ...
    def generate_e7_settings(
        ...
        cap_resource_map = self.generate_cap_resource_map()
        ...
        cap_target_bpc: dict[str, TargetBPC] = {
            target_name: TargetBPC(
                box=self.driver.box[m["box"].box_name],
    ...
    def execute(
        ...
        if self._driver is None:
            raise ValueError("driver is not defined")
        # quel1system = (
        #     self.create_quel1system(boxpool) if self._driver is None else self.driver
        # )
        quel1system = self.driver
        # c, g, m = self.generate_e7_settings()
        ...
        # return settings
        # sys.exit()  # TODO: breakpoint for debug

        settings, _ = self.convert_sequence_to_settings()
        ...
        # quelware 0.10  対応コード
        # 内部では quelware4 向けの実装が呼ばれている
        commands, waveforms = convert_settings_to_params(settings)
        results = run(
            quel1system,
            band_refs=[w.band for w in waveforms],
            waveforms=waveforms,
            commands=commands,
        )
        # TODO 結果を qubecalib3 互換形式に変換するコードを書く
        return results

    def convert_sequence_to_settings(
        self,
    ) -> tuple[
        list[direct.RunitSetting | direct.AwgSetting | direct.TriggerSetting],
        dict[str, Any],
    ]:
        quel1system = self.driver
    ...
    def step_execute(
        ...
        if self.quel1system is None:
            ...
            for box_name in self.quel1system.boxes:
                ...
                sqc = SequencerClient(box.wss._ipaddr_wss)
...
class BoxPool:
    ...
    def __init__(self) -> None:
        self._clock_master: QuBEMasterClient | None = (
