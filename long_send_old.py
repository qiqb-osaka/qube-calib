...
def set_wave_sequence(awg_ctrl, amps, freqs):
...
def start(amps, freqs, ipaddr='10.0.0.16'):
    ...
    awg_ctl = e7awgsw.AWG.all()
    # 初期化
    awg_ctrl.initialize(*awg_ctl)
    #init_modules(awg_ctrl, cap_ctrl)
    # 波形シーケンスの設定
    awg_to_wave_sequence = set_wave_sequence(awg_ctrl, amps, freqs)
    # 波形送信スタート
    awg_ctrl.start_awgs()

def stop(ipaddr='10.0.0.16'):
    awg_ctrl = e7awgsw.AwgCtrl(ipaddr)
    awg_ctrl.terminate_awgs(*e7awgsw.AWG.all())
