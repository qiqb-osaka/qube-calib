...
class LMX2594(LSI):
    ...
    def mhz(self, v):
        ...
        if v % 100:
            warnings.warn(
                "The frequency can only be set in multiples of 100MHz. {}MHz is set.".format(
                    mhz * 100
                ),
                stacklevel=2,
            )
        ...
        if mhz * 100 % 500:
            warnings.warn(
                "To synchronize the output phase, the frequency must be a multiple of 500MHz.",
                stacklevel=2,
            )
...
class ConfigFPGA(object):
    ...
    def config(cls, bitfile: str) -> None:
        ...
        commands = [
            ...
            "batch",
            "-source",
            "{}/utils/config.tcl".format(PATH_TO_ADIAPIMOD),
        ]
