...
import e7awgsw
import numpy as np
from e7awgsw import AwgCtrl, CaptureModule, CaptureParam
...
from .pulse import Arbitrary, Blank, Read, Schedule, SlotWithIQ
from .qube import AWG, CPT, SSB, Monitorin, Readin, Readout
...
def words(t):  # in ns
    return int(
        t * 1e-9 * AwgCtrl.SAMPLING_RATE // e7awgsw.hwparam.NUM_SAMPLES_IN_AWG_WORD
    )
    ...
    for slot in channel:
        ...
        if not isinstance(slot, Blank):
            r.append(
                RangeWithSlot(
                    range=(cursor, tail),
                    slots=[RangeWithSlot(range=(cursor, tail), slots=slot)],
                )
            )
        cursor = tail
    return r
...
def merge_slots(rng, sampling_rate):
    ...
    rslt = RangeWithSlot(
        range=rng.range,
        slots=Arbitrary(
            duration=rng.range[1] - rng.range[0],
            amplitude=1,
...
def quantize_blank(channel):
    ...
    while True:
        ...
        if isinstance(slot, Blank):
            ...
            div, mod = divmod(
                shorten_duration, 8
            )  # Blank は 8ns の整数倍にしなければいけない
            if mod:  # 余りがある
                if (
                    not div
                ):  # 8ns より短い場合は直後の Slot に吸収してもらわなければいけない
        ...
        elif isinstance(slot, Padding) or isinstance(
            slot, Shortage
        ):  # Padding と Shortage は次に処理されるはず
            pass
        else:
...
def quantize_wave(channel):
    ...
    for i in range(len(channel)):
        ...
        if isinstance(slot, Arbitrary):
            ...
            new.iq = np.concatenate(
                [np.zeros(int(padding_duration / 2)).astype(complex), slot.iq]
            )
            r.append(new)
            ...
            if m:
                ...
                new.iq = np.concatenate(
                    [new.iq, np.zeros(int(extention / 2)).astype(complex)]
                )
                # お尻に追加したブランクデータ長分だけはみ出したことを記録する
                r.append(
                    Shortage(duration=new.duration - (slot.duration + padding_duration))
                )
...
def quantize_channel(channel):
    ...
    while True:
        ...
        if not list(
            filter(lambda x: isinstance(x, Shortage) or isinstance(x, Padding), buf)
        ):
            break
    for e in buf:
        rslt.append(e)
...
def merge_channels(channels):
    ...
    m = max_sampling_rate = max(
        [slot.sampling_rate for chnl in channels for slot in chnl]
    )
...
def conv_to_waveseq_and_captparam(schedule, repeats=1, interval=100000):
    ...
    qube_ipfpgas = list(
        set(
            [
                k.port.capt.ipfpga if isInputPort(k.port) else k.port.awg0.ipfpga
    ...
    w2c = dict(
        [(k, conv_channel_for_e7awgsw(v, schedule.offset)) for k, v in channels.items()]
    )
    # すべてのチャネルの全長を揃える
    ...
    def conv_channel_to_e7awgsw_wave_sequence(c):
        ...
        for i in range(1, len(c), 2):
            w.new_chunk(
                duration=c[i].duration * 1e-9, amp=int(c[i].amplitude * 32767), blank=0
            )
            w.chunk[-1].iq[:] = c[i].iq[:]
    ...
    def conv_channel_to_e7awgsw_capture_param(channel):
        ...
        for i in range(0, len(c), 2):
            if (isinstance(c[i], Read) or isinstance(c[i], Arbitrary)) and isinstance(
                c[i + 1], Blank
            ):
...
def _conv_channel_for_e7awgsw(channels, awg_capt, offset_time):
    ...
    # 周波数領域にチャネルを配置する
    modulated_channels = [
        _modulation(channel, awg_capt, offset_time) for channel in channels
    ]
    # 時間領域でチャネルを結合する
    merged_channel = merge_channels(modulated_channels)
    # e7awgswの制約に合わせてスロットを再配置する
...
def _conv_to_e7awgsw(
    adda_to_channels, offset=0, repeats=1, interval=100000, trigger_awg=None
):
    # どの ipfpga の qube が使われているかリストする
    ...
    for qube in qubes:
        ...
        qube_to_e7awgsw[qube] = _conv_to_e7awgsw_single(
            qube, channels, offset, repeats, interval, trigger_awg
        )[qube]
...
def _conv_to_e7awgsw_single(qube, channels, offset, repeats, interval, trigger_awg):
    ...
    w2c = dict(
        [(k, _conv_channel_for_e7awgsw(v, k, offset)) for k, v in channels.items()]
    )
    qube_to_e7awgsw[qube]["awg_to_mergedchannel"] = {
        k: v for k, v in w2c.items() if isinstance(k, AWG)
    }
    qube_to_e7awgsw[qube]["capt_to_mergedchannel"] = {
        k: v for k, v in w2c.items() if isinstance(k, CPT)
    }
    # すべてのチャネルの全長を揃える
    m = max([v.duration for k, v in w2c.items()])
    for k, v in w2c.items():
    ...
    def conv_channel_to_e7awgsw_capture_param(channel, delay):
        ...
        for i in range(0, len(c), 2):
            ...
            sum_sections.append(SumSection(capture_words, blank_words))
        s = sum_sections[-1]
        sum_sections[-1] = SumSection(
            s.capture_words, s.blank_words + words(interval) + final_blank_words
        )
        # print(sum_sections)
...
def conv_to_e7awgsw(schedule, repeats=1, interval=100000, trigger_awg=None):
    ...
    ipfpga_to_e7awgsw[ipfpga]["awg_to_wave_sequence"] = {
        f.port.awg0: s for f, s in send
    }
    ipfpga_to_e7awgsw[ipfpga]["capt_module_to_capt_param"] = {
        f.port.capt: s for f, s in recv
    }
    # トリガを設定する
    # Readout_send の Wire のリストを得る
    if trigger_awg is None:
        readout_sends = list(
            set([k for k, v in channels.items() if isinstance(k.port, Readout)])
        )
        if readout_sends:
            trigger_awg = readout_sends[0].port.awg0
        else:
            # モニタ経路を使う場合必ずしも Readout がある訳ではない
            trigger_awg = [
                w.port.awg0 for w, c in w2c.items() if not isInputPort(w.port)
            ][0]
...
def retrieve_data_into_mergedchannel(capt_to_mergedchannel, recv, offset=0):
    ...
    for w, c in w2c.items():
        ...
        c.timestamp = t_ns = (
            np.linspace(0, d, int(d * m * 1e-9), endpoint=False).astype(int) - offset
        )
...
def _demodulate(schedule, capt_to_mergedchannel, recv, adda_to_channels, offset=0):
    ...
    key_to_mergedchannel = {
        channel_to_key(w): capt_to_mergedchannel[k]
        for k, v in capt_to_channels.items()
        for w in v
    }
    ...
    for k, c in schedule.items():
        is_Read_in_Channel = (
            np.sum([True if isinstance(o, Read) else False for o in c]) == True
        )
...
def demodulate(schedule, e7awgsw_setup, recv):
    ...
    for w, c in w2c.items():
        ...
        m = max([s.sampling_rate for s in c])
        c.timestamp = t_ns = (
            np.linspace(0, d, int(d * m * 1e-9), endpoint=False).astype(int) - s.offset
        )
        c.iq = np.zeros(len(t_ns)).astype(complex)
    ...
    for k, c in schedule.items():
        ...
        for v in c:
            ...
            v.global_timestamp = t = t_ns * 1e-9
            v.iq = np.zeros(len(t_ns)).astype(complex)
            v.iq[:] = w2c[c.wire].iq[idx] * np.exp(
                -1j * 2 * np.pi * calc_modulation_frequency(c) * t
            )
