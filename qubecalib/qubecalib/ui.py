...
def lmx2594_ad9082_do_init(self, ad9082_mode=True, readout_mode=False):
    ...
    if ad9082_mode:
        ...
        self.write_value(
            0x47, 0x008D
        )  # R71 [7:5]SYSREF_DIV_PRE=4='Divided by 4', [3]SYSREF_EN=1, [2]SYSREF_REPEAT=1
    else:
        self.write_value(0x48, 0x0000)  # R72 [10:0] SYSREF_DIV
        self.write_value(
            0x47, 0x0081
        )  # R71 [7:5]SYSREF_DIV_PRE=4='Divided by 4', [3]SYSREF_EN=0, [2]SYSREF_REPEAT=0
    ...
    if ad9082_mode:
        self.write_value(
            0x3A, 0x0401
        )  # R58 [15]IGNORE=0, [14]HYST=0, [13:12]INPIN_LVL=0=V
    else:
        self.write_value(
            0x3A, 0x8001
        )  # R58 [15]IGNORE=0, [14]HYST=0, [13:12]INPIN_LVL=0=V
...
def boot_fpga_from_rom(qube: lib_qube.QubeBase) -> Any:
    os.environ["ADAPTER"] = qube.adapter_au50
    ROOT = Path(__file__).parent / Path("tcl/reboot_from_rom.tcl")
    cmd = "vivado -mode batch -source {}".format(ROOT).split(" ")
...
class QubeControl(object):
    def __init__(self, config_file_name, qube=None):
        ...
        c["mon"] = ipw.ToggleButtons(
            description="ADC0", options=["Readin1", "Monitor4"], disabled=False
        )
        c["mon2"] = ipw.ToggleButtons(
            description="ADC1", options=["Readin12", "Monitor9"], disabled=False
        )
        c["loopback"] = ipw.Checkbox(
            value=False, description="Switch Loopback", disabled=False
        )
        c["maxtryad9082"] = ipw.IntText(
            value=5,
            description="Max Try:",
            disabled=False,
            layout=ipw.Layout(width="192px"),
        )
        ...
        class RestartAD9082Button(ipw.Button):
            ...
            def _on_click(self, e):
                    ...
                    for o, k in zip(c["qube"].ad9082, ["mon", "mon2"]):
                        if c[k].value.startswith("Monitor"):
                            ...
                            ret = subprocess.check_output(
                                "{}/v1.0.6/src/hello_monitor".format(o.path),
                                encoding="utf-8",
                            )
                        else:
                ...
                def check_recv(prx1, prx2):
                    ...
                    w = [
                        r.data.data[meas.CaptureModule.get_units(prx.capt.id)[0]]
                        for prx in [prx1, prx2]
                    ]

                    sgm2 = [(np.abs(v - v.mean()) ** 2).mean() for v in w]
                    print("Variance: ", sgm2)
                    if [s < 1e8 for s in sgm2] == [True, True]:
                        return True
                    else:
                        print(
                            "An anomalous variance has been detected. Trying the restart process again..."
                        )
                        return False
                ...
                with wout:
                    for i in range(max_try):
                        ...
                        for o in c["qube"].ad9082:
                            print(
                                dict(o.get_jesd_status())["0x55E"], end=" ", flush=True
                            )
                        s = [
                            dict(c["qube"].ad9082[i].get_jesd_status())["0x55E"]
                            == "0xE0"
                            for i in range(2)
                        ]
                        if s != [True, True]:
                        ...
                        prx1 = (
                            qube.port4
                            if c["mon"].value.startswith("Monitor")
                            else qube.port1
                        )
                        prx2 = (
                            qube.port9
                            if c["mon2"].value.startswith("Monitor")
                            else qube.port12
                        )
        ...
        class ShowRecvButton(ipw.Button):
            ...
            def _on_click(self, e):
                assert [
                    dict(qube.ad9082[i].get_jesd_status())["0x55E"] == "0xE0"
                    for i in range(2)
        ...
        class ConfigFPGAButton(ipw.Button):
            ...
            def _on_click(self, e):
                ...
                # c['qube'].config_fpga()
                os.environ["BITFILE"] = "{}/{}".format(
                    lib_qube.PATH_TO_BITFILE, c["qube"].bitfile
                )
                with c["wout"]:
    ...
    def qube(self):
        if self._container["qube"] == None:
            raise ValueError(
                "An instance of qube must be created before it can be referenced."
            )
