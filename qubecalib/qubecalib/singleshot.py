...
#    
#    def __init__(self, ip_addr, port, *dests): # ipmulti
#        
#        self.master = qubemasterclient.QuBEMasterClient(ip_addr, port)
#        self.port = port
#        self.dests = dests
#        self.cur_time = 0
#        
...
#        c = self.master
#        print('clear')
#        r, a = c.clear_clock(value=0)
#        print('kick')
#        r, a = c.kick_clock_synch(self.dests)
#        print('read')
#        self.cur_time = cur_time = c.read_clock(value=0)
#        print('read result', cur_time, 'cycles')
#        
#    def start(self, wait_sec):
#        
#        s = int(wait_sec * 125000000) # 125Mcycles = 1sec
#        d = self.dests
#        
#        self.cur_time += s
#        ct = self.cur_time
...
#        
#        for a in d:
#            c = sequencerclient.SequencerClient(a, self.port)
#            r, a = c.add_sequencer(ct) # make registration
#            

def neorun(adda_to_channels, triggers, repeats=1, timeout=30, interval=50000):
    ...
    if len(c.keys()) == 1:
        ...
        units = neorun_single(qube, qube_to_pulse[qube], trigger, repeats, timeout, interval)
    else:
        units = neorun_multi(qube_to_pulse, triggers, repeats, timeout, interval)
...
def neorun_multi(pulse, triggers, repeats=1, timeout=30, interval=50000):
    ...
    for qube, v in pulse.items():
        for captm, w in v.capt_to_captparam.items():
            ...
            neorun_get_data(captm, channels)
    
    return units

def neorun_single(qube, pulse, trigger, repeats=1, timeout=30, interval=50000):
...
def neorun_get_data(captm, channels):
